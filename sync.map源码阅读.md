...
> 自己项目中使用了sync.Map，但是引起性能问题，后面改为管道方式，显示的提升了性能。
>
> 使用sync.Map引发的问题
>
> 1. gc heap频繁发生
...
In these two cases, use of a Map may significantly reduce lock contention compared to a Go map paired with a separate Mutex or RWMutex（有限的应用场景，很多场景并不适合使用sync.Map，用错了场景反而会引起性能下降。）

只有在上面两种场景下，相比使用普通map和Mutex或者RWMutex的组合，sync.Map才会显著减少锁竞争。
...
`mu` 对dirty map时进行加锁操作

`read`对readOnly进行存储数据，提供原子操作，读取或者修改时可不加锁。注意该map中存储了（*之前存储后面删除，且置换为expunge的状态值*），但是dirty中不会存储

> 将readOnly中曾经存在但是后期删除的值，标识为expunge的时机 => 存储了readOnly中不存在需要往dirty map中存储时
>
> 1. 将新值放入dirty中
> 2. 判断readOnly中是否存在曾经存在但是后期删除的值，如果存在，将值置换为expunge标识
> 3. 将readOnly中非expunge值全部存储到dirty中
...
优先从readOnly中读取（不需要mu lock，从atomic.Value()中直接读取map中直接获取）
...
- LoadAndDelete
  ...
  >
  > 错误做法`syncMap.Store("1", nil)`，sync.Map存储的是value的指针值，也就是指向nil的指针值并不为0x0
  >
...
/**
dirtyLocked 主要是会将readOnly.m中的nil值置换为expunged，dirty中不存储任何expunge的值。

什么情况下readOnly.m中会存储nil的值呢？也就是Store后直接更新为nil？更新为nil还是有一个具体地址。
...
sync.Map降低并发的关键在于 readOnly和dirty的搭配。readOnly 用到了atomic.Value不需要使用锁。能显著降低锁竞争，提高性能。
...
> 没有必要存储一堆没用的数据，占用额外的内存，后面dirty数据都平均读取一遍及其以上后，dirty数据会重新覆盖read数据，那个时候read中expunge会被擦除。
